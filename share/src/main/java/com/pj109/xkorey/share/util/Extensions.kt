...
import com.pj109.xkorey.share.BuildConfig.APIVERSION
import com.pj109.xkorey.share.BuildConfig.PROJECTCODE
...
inline fun defaultReq(path:String): Request {
    val req = Request()
    req.url = BuildConfig.APIHOST+path
    req.params = hashMapOf()
    if(cacheOut("what's your","name").isNullOrBlank()){
        req.header = hashMapOf("projectCode" to PROJECTCODE,"api_version" to APIVERSION)
    }else{
        req.header = hashMapOf("projectCode" to PROJECTCODE,"api_version" to APIVERSION,"key" to cacheOut("what's your","name"))
    }
    return req
}


inline fun cacheOut(key:String,type:String):String{
      return Staff.cache(key+type)
}

inline fun cacheIn(key:String,type:String,value:String){
    Timber.i("data in cache:key ${key} value ${value}")
    Staff.cache(key+type,value)
}


inline fun oCacheOut(key:String,type:String):LiveData<String>{
    return Staff.observerCache(key+type)
}

inline fun rndStr():String{
    return UUID.randomUUID().toString()
}

inline fun oCacheIn(key:String,type:String,value:String){
    Timber.i("observer data in :key ${key} value ${value}")
    Staff.observerCache(key+type,value)
}

@Throws(InterruptedException::class)
inline fun <T> waitLivedata(liveData: LiveData<T>,waitmin:Long): T {
    ...
    latch.await(waitmin, TimeUnit.SECONDS)
