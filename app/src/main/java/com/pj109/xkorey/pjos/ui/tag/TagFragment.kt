...
import com.pj109.xkorey.pjos.databinding.FragmentTagBinding
...
import com.pj109.xkorey.pjos.widget.SpaceDecoration
import com.pj109.xkorey.share.util.activityViewModelProvider
import dagger.android.support.DaggerFragment
import timber.log.Timber
import javax.inject.Inject

class TagFragment : DaggerFragment() {

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        binding=FragmentTagBinding.inflate(inflater,container,false).apply {
            headerAlpha = this@TagFragment.headerAlpha
            descriptionAlpha = this@TagFragment.descriptionAlpha
        }
        return binding.root
    }
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        tagViewModel=activityViewModelProvider(viewModelFactory)
        binding.viewModel = tagViewModel
        behavior = BottomSheetBehavior.from(binding.filterSheet)
        tagAdapter= TagAdapter(tagViewModel)
        tagViewModel.tagFilters.observe(this, Observer {
            tagAdapter.submitEventFilterList(it)
        })
        ...
        binding.recyclerview.apply {
            ...
            (layoutManager as GridLayoutManager).spanSizeLookup =
                ScheduleFilterSpanSizeLookup(tagAdapter)
...
fun selectedFilters(recyclerView: RecyclerView, filters: List<EventFilter>?) {
    var filterChipAdapter: FilterChipAdapter
    if (recyclerView.adapter == null) {
        filterChipAdapter = FilterChipAdapter((filters ?: emptyList()))
        recyclerView.apply {
...
fun setClickListenerForFilter(
    ...
    filter.setOnClickListener {
        if (eventFilter is EventFilter.MyEventsFilter && !viewModel.isSignedIn()) {
            Timber.i("not function in here")
        } else {
